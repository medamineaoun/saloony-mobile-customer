...
class SalonsByCategoryPage extends StatefulWidget {
  final SalonCategory category;

  const SalonsByCategoryPage({super.key, required this.category});

  @override
  State<SalonsByCategoryPage> createState() => _SalonsByCategoryPageState();
}

class _SalonsByCategoryPageState extends State<SalonsByCategoryPage> {
  ...
  void initState() {
    ...
    _salonsFuture = _loadSalonsByCategory();
  }

  Future<List<Salon>> _loadSalonsByCategory() async {
    try {
      ...
      if (result['success']) {
        ...
        if (salons is List) {
          final filtered = List<Salon>.from(salons)
              .where((salon) => salon.salonCategory == widget.category)
              .toList();
          return filtered;
        }
      }
